...
def getTranslate(transform: str) -> (float, float):
    m = re.search(r"translate\((-*\d*.\d*),(-*\d*.\d*)\)", transform)
    xpos, ypos = float(m.group(1)), float(m.group(2))
    return xpos, ypos
